...
export default async function pollingRoutes(appBase: FastifyInstance) {
  ...
  app.post(
    '/repos/:id/poll',
    {
      schema: {
        params: IdParams,
        // `reviewTriggered` is always false — the field documents that polling
        // syncs the PR list and never starts a review (that trigger is manual).
        response: {
          200: z.object({ synced: z.number().int(), reviewTriggered: z.boolean() }),
          ...ApiErrors,
          ...NotFound,
        },
      },
