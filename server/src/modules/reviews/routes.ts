...
export default async function reviewsRoutes(appBase: FastifyInstance) {
  ...
  app.post(
    '/pulls/:id/review',
    {
      schema: { params: IdParams, response: { 200: ReviewRunResponse, ...ApiErrors, ...NotFound } },
  ...
  // The ONE route in the API with no `response:` schema, and the only one that
  // can't have one: `reply.sse()` hijacks the reply and streams `text/event-
  // stream` frames, so there is no single JSON body to serialize. The frame
  // payload is a `RunEvent` — validated where it is published, not here.
  app.get(
    '/runs/:id/events',
  ...
  app.get(
    ...
    { schema: { params: IdParams, response: { 200: z.array(ActiveRun), ...ApiErrors } } },
    async (req) => {
      const { workspaceId } = await getContext(container, req);
  ...
  app.get(
    '/pulls/:id/runs',
    { schema: { params: IdParams, response: { 200: z.array(RunSummary), ...ApiErrors } } },
    async (req) => {
      const { workspaceId } = await getContext(container, req);
  ...
  app.delete(
    '/runs/:id',
    { schema: { params: IdParams, response: { 200: OkResponse, ...ApiErrors } } },
    async (req) => {
      const { workspaceId } = await getContext(container, req);
  ...
  app.get(
    '/runs/:id/trace',
    { schema: { params: IdParams, response: { 200: RunTrace, ...ApiErrors, ...NotFound } } },
    async (req) => {
      await getContext(container, req);
  ...
  app.get(
    '/pulls/:id/reviews',
    { schema: { params: IdParams, response: { 200: z.array(ReviewRecord), ...ApiErrors, ...NotFound } } },
    async (req) => {
      const { workspaceId } = await getContext(container, req);
  ...
  for (const action of FINDING_ACTIONS) {
    app.post(
      `/findings/:id/${action}`,
      {
        schema: {
          params: IdParams,
          response: { 200: z.object({ finding: FindingRecord }), ...ApiErrors, ...NotFound },
        },
      },
      async (req) => {
        const { workspaceId } = await getContext(container, req);
