...
export class ReviewRunExecutor {
  ...
  async executeRuns(
    ...
    // Trace first, status second — the same order as the per-run paths below.
  ...
  private async runOneAgent(
    ...
    try {
      ...
      // The trace is persisted BEFORE the run turns terminal, so anything that
      // waits for `done` (the UI's trace drawer, tests, the MCP) can read it.
      ...
      // Trace first, status second — same order as the success path.
      await this.repo
