 ...
 * `agent_runs` + `run_traces` (one trace doc per run). `pr_intent` moved to
 * `modules/intent/repository.ts` (L03 D1) — this class no longer touches it.
...
export class ReviewRepository {
  ...
  completeAgentRun(
    ...
    values: {
      ...
      tokensIn: number;
      tokensOut: number;
      /** Estimated USD spend; null when the model has no known price. */
      costUsd: number | null;
