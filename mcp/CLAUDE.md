...
What it is and the 5 tools: [`README.md`](README.md) (added in a later slice).
...
Instructions and per-tool descriptions are capped (§6 of the spec) and
enforced by `test/tools-list.test.ts` — check that test before growing one.

## `.context/`

`docs/` reference · `specs/` planned changes · `insights/INSIGHTS.md`
committed findings, read and appended by the `engineering-insights` skill.
