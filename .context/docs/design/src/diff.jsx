...
function DiffFileCard({ file, role }) {
  ...
  return React.createElement("div", { style: { border: "1px solid var(--border)", borderRadius: 7, overflow: "hidden", background: "var(--bg-elevated)" } },
    ...
    open && lines && React.createElement("div", { style: { borderTop: "1px solid var(--border)", padding: "6px 0", background: "var(--bg-surface)" } },
      lines.map((ln, i) => React.createElement(CodeLine, { key: i, ln }))),
    ...
    smart ? D.groups.map((g, gi) => {
      ...
      return React.createElement("div", { key: gi, style: { marginBottom: 18 } },
        ...
        React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } },
          g.files.map((f, fi) => React.createElement(DiffFileCard, { key: fi, file: f, role: g.role }))));
    }) : React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } },
      D.groups.flatMap((g) => g.files).sort((a, b) => a.path.localeCompare(b.path)).map((f, fi) => React.createElement(DiffFileCard, { key: fi, file: f }))));
}
